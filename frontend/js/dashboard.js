document.addEventListener('DOMContentLoaded', async () => {
    ...
    const checkWhatsAppStatus = async (showToast = false) => {
        try {
            ...
            // Handle Cloud API mode (Vercel production)
            if (data.mode === 'cloud') {
                waPollActive = false; // no rapid polling needed for Cloud API
                ...
                if (data.ready || data.status === 'connected' || data.status === 'configured') {
                    statusText.innerHTML = `<span style="color: #10B981; font-weight: 600;">✅ OpenWA WhatsApp Ready</span><br><small style="color: #6B7280; font-size: 0.8rem;">Railway WhatsApp service active</small>`;
                    if (showToast) showWaToast('✅ OpenWA WhatsApp service is connected and ready', 'success');
                } else {
                    statusText.innerHTML = `<span style="color: #F59E0B; font-weight: 600;">⚙️ OpenWA Setup Required</span><br><small style="color: #6B7280; font-size: 0.8rem;">Add <strong>OPENWA_API_KEY</strong> and <strong>OPENWA_API_URL</strong> in Vercel Settings.</small>`;
                    if (showToast) showWaToast('⚙️ OpenWA environment variables not configured in Vercel', 'warning');
                }
                return;
            }

            // Handle Local mode (whatsapp-web.js with QR code pairing)
            if (data.ready) {
                statusText.innerHTML = '<span style="color: #10B981; font-weight: 600;">✅ Linked and Ready</span><br><small style="color: #6B7280; font-size: 0.8rem;">Local WhatsApp session active</small>';
                qrContainer.style.display = 'none';
                refreshBtn.style.display = 'inline-block';
                if (logoutBtn) logoutBtn.style.display = 'inline-block';
                if (showToast) showWaToast('✅ WhatsApp Status: Linked and Ready', 'success');
            } else if (data.qr) {
                statusText.innerHTML = '<span style="color: #F59E0B; font-weight: 600;">⚠️ Scan QR Code to link WhatsApp</span>';
                qrImg.src = data.qr;
    ...
    // Auto-refresh WA status every 5 seconds — only when in local mode and still initializing/scanning
    setInterval(() => {
        if (!waPollActive) return;
